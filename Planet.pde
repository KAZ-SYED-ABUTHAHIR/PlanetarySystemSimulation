public class Planet {
  ...
  ArrayList<PVector> orbitVelocities  = new ArrayList<PVector>();
  ArrayList<PVector> orbitAccelerations  = new ArrayList<PVector>();
  ArrayList<PolarPoint> orbitPolarPoints = new ArrayList<PolarPoint>();
  int orbitPointIndex = 0;
  ...
  /* KEPLER'S SWEEP PARAMS */

  boolean sweepRadial = false;
  float sweepRadialNumOfPoints;
  int numOfArealSweeps = 16;
  ArrayList<ArealTriangle> sweepTriangles = new ArrayList<ArealTriangle>();
  ...
  final static float accelerationArrowScale = 2500;

  boolean highlighted = false;

  PImage img = null ;

  public float rotAngle = 0.0f; // Angle to rotate the planet

  public boolean calculateInstantaneously = false;
  ...
  void show() {
    if (this.img != null) {
      ...
      this.rotAngle = atan2(this.myAttractor.position.y - this.position.y, this.myAttractor.position.x - this.position.x);
      ...
      image(this.img, 0, 0, img.width/1.6, img.height/1.6);
      popMatrix();
    } else {
      pushStyle();
  ...
  private void sweepArea() {
    if (this.sweepRadial && this.highlighted) {
      int numOfSweepTriangles = this.sweepTriangles.size();
      float numOfOrbitPointsByScale = this.orbitPoints.size()/2.0f;

      if (frameCount%1==0 && int(frameCount/sweepRadialNumOfPoints)%2 == 1 && numOfSweepTriangles < numOfOrbitPointsByScale+6) {
        PVector currentPos = new PVector(this.position.x, this.position.y);
        PVector oldPos = new PVector(this.oldPosition.x, this.oldPosition.y);
        this.sweepTriangles.add(new ArealTriangle(this.myAttractor.position, oldPos, currentPos));
      }

      for (ArealTriangle sweepTriangle : this.sweepTriangles) {
        sweepTriangle.show(this.planetColor, 100);
      }
    }
  }
  ...
  void update() {
    if (calculateInstantaneously) {
      //NEW VERLET
      velocityVerletIntegrate(deltaTime); // Velocity Verlet is More Accurate and Stable.
      ...
      try {
        this.orbitPointIndex++ ;

        this.oldPosition = this.orbitPoints.get(orbitPointIndex-1).copy();
        this.position = this.orbitPoints.get(orbitPointIndex).copy();
        this.velocity = new PVector(this.orbitVelocities.get(orbitPointIndex).x, this.orbitVelocities.get(orbitPointIndex).y);
        this.acceleration = new PVector(this.orbitAccelerations.get(orbitPointIndex).x, this.orbitAccelerations.get(orbitPointIndex).y);

        velocityArrow.setArrow(this.position, this.velocity.copy().mult(velocityArrowScale));
        accelerationArrow.setArrow(this.position, this.acceleration.copy().mult(velocityArrowScale*100));

        if (this.orbitPointIndex > this.orbitPoints.size()-2) {
          this.orbitPointIndex = -1;
        }
      }
      catch(Exception e) {
        e.printStackTrace();
  ...
  void initUpdate() {
    ...
    //velocityArrow.setArrow(this.position, this.velocity.copy().mult(velocityArrowScale));
    //accelerationArrow.setArrow(this.position, this.acceleration.copy().mult(velocityArrowScale*30));
    this.orbitAccelerations.add(this.acceleration.copy());
    this.acceleration.setMag(0);
  }
  ...
  private void velocityVerletIntegrate(float deltaTime) {
    PVector tempPosition = this.position.copy();

    // Position update
    PVector halfVelocity = PVector.add(velocity, PVector.mult(acceleration, deltaTime / 2.0f));
    PVector newPosition = PVector.add(position, PVector.mult(halfVelocity, deltaTime));

    // Force update (calculate new acceleration)
    PVector newAcceleration = calcNewAcceleration(myAttractor, newPosition);

    // Velocity update
    PVector averageAcceleration = PVector.add(acceleration, newAcceleration).mult(0.5f);
    PVector newVelocity = PVector.add(velocity, PVector.mult(averageAcceleration, deltaTime));
  ...
  private PVector calcNewAcceleration(Attractor attractor, PVector newPosition) {
    float dist = PVector.dist(newPosition, attractor.position);
    PVector radialVector = PVector.sub(attractor.position, newPosition).setMag(1);
    return radialVector.setMag(G * attractor.mass / (dist * dist));
  }
  ...
  void calcOrbit(Attractor attractor) {
    ...
    do {
      ...
      this.orbitVelocities.add(vel);
      // acceleration is updated in the initUpdate Method before setting its magnitude to zero.
    ...
    sweepRadialNumOfPoints = (this.orbitPoints.size()/this.numOfArealSweeps) ;
  }
  ...
  void showRadial(Attractor _attractor) {
    pushStyle();
    stroke(this.planetColor);
    strokeWeight(5);
    line(_attractor.position.x, _attractor.position.y, this.position.x, this.position.y);
    popStyle();
  }
  ...
  void showAcceleration(PVector anchor) {
    this.accelerationArrow.setArrow(anchor, this.acceleration.copy().mult(accelerationArrowScale));
    this.accelerationArrow.show();
  }

  void showVelocity(PVector anchor) {
    this.velocityArrow.setArrow(anchor, this.velocity.copy().mult(velocityArrowScale));
    this.velocityArrow.show();
  }
  ...
  void setSweepRadial(boolean _sweepRadial) {
    this.sweepRadial = _sweepRadial;
  }
}//CLASS_END
